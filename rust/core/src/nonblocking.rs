...
pub trait AsyncDriver: Send {
    ...
    fn new_database(
        &mut self,
    ) -> impl std::future::Future<Output = Result<Self::DatabaseType>> + Send;
...
pub trait AsyncDatabase: AsyncOptionable<Option = OptionDatabase> + Send {
    ...
    fn new_connection(
        &self,
    ) -> impl std::future::Future<Output = Result<Self::ConnectionType>> + Send;
...
pub trait AsyncConnection: AsyncOptionable<Option = OptionConnection> + Send {
    ...
    fn new_statement(
        &mut self,
    ) -> impl std::future::Future<Output = Result<Self::StatementType>> + Send;

    /// Cancel the in-progress operation on a connection.
    fn cancel(&mut self) -> impl std::future::Future<Output = Result<()>> + Send;
    ...
    fn get_info(
        ...
        Output = Result<Pin<Box<dyn arrow_array::nonblocking::RecordBatchStream + Send>>>,
    > + Send;
    ...
    fn get_table_schema(
        ...
        db_schema: Option<&str>,
        table_name: &str,
    ) -> impl std::future::Future<Output = Result<Schema>> + Send;
    ...
    fn commit(&mut self) -> impl std::future::Future<Output = Result<()>> + Send;

    /// Roll back any pending transactions. Only used if autocommit is disabled.
    ///
    /// Behavior is undefined if this is mixed with SQL transaction statements.
    fn rollback(&mut self) -> impl std::future::Future<Output = Result<()>> + Send;
...
pub trait AsyncStatement: AsyncOptionable<Option = OptionStatement> + Send {
    ...
    /// statements.
    fn bind(&mut self, batch: RecordBatch) -> impl std::future::Future<Output = Result<()>> + Send;
    ...
    fn bind_stream(
        &mut self,
        reader: Pin<Box<dyn arrow_array::nonblocking::RecordBatchStream + Send>>,
    ) -> impl std::future::Future<Output = Result<()>> + Send;
    ...
    fn execute_update(&mut self) -> impl std::future::Future<Output = Result<Option<i64>>> + Send;
    ...
    ///
    /// ADBC API revision 1.1.0
    fn execute_schema(&mut self) -> impl std::future::Future<Output = Result<Schema>> + Send;

    /// Execute a statement and get the results as a partitioned result set.
    fn execute_partitions(
        &mut self,
    ) -> impl std::future::Future<Output = Result<crate::PartitionedResult>> + Send;
    ...
    fn get_parameter_schema(&self) -> impl std::future::Future<Output = Result<Schema>> + Send;
