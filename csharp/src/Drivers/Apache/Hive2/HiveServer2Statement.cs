    ...
    internal abstract class HiveServer2Statement : AdbcStatement
    {
        ...
        private async Task<QueryResult> ExecuteQueryAsyncInternal(CancellationToken cancellationToken = default)
        {
            return await TraceActivity(async activity =>
            {
                ...
                await HiveServer2Connection.PollForResponseAsync(OperationHandle!, Connection.Client, PollTimeMilliseconds, ActivitySource, cancellationToken); // + poll, up to QueryTimeout
