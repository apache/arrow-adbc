...
namespace Apache.Arrow.Adbc.Tests.Drivers.Databricks
{
    ...
    public class CloseOperationExperimentTests : TestBase<DatabricksTestConfiguration, DatabricksTestEnvironment>
    {
        ...
        /// <summary>
        /// Test 3: Check operation status before and after CloseOperation
        /// </summary>
        [SkippableFact]
        public async Task Test3_CheckOperationStatus_BeforeAndAfterClose()
        {
            OutputHelper?.WriteLine("=== Test 3: Operation status check ===");
            ...
            try
            {
                ...
                statement.SqlQuery = @"
                    SELECT COUNT(*) as cnt
                    FROM (
                        SELECT t1.id + t2.id as sum_id
                        FROM RANGE(100000) t1
                        CROSS JOIN RANGE(100000) t2
                    ) WHERE MOD(sum_id, 7) = 0";

                OutputHelper?.WriteLine($"Starting query at {DateTime.Now:HH:mm:ss.fff}");
                ...
                if (statement.OperationHandle != null)
                {
                    var statusReq = new TGetOperationStatusReq(statement.OperationHandle);
                    var statusResp = await connection.Client.GetOperationStatus(statusReq);
                    OutputHelper?.WriteLine($"Status before close: {statusResp.OperationState}");
                }
                
                // Close the operation
                OutputHelper?.WriteLine($"Calling CloseOperation at {DateTime.Now:HH:mm:ss.fff}");
                var closeResponse = await statement.TestCloseOperationAsync();
                OutputHelper?.WriteLine($"CloseOperation response: {closeResponse.Status?.StatusCode}");
                
                // Try to check status after close (might fail)
                try
                {
                    if (statement.OperationHandle != null)
                    {
                        var statusReq = new TGetOperationStatusReq(statement.OperationHandle);
                        var statusResp = await connection.Client.GetOperationStatus(statusReq);
                        OutputHelper?.WriteLine($"Status after close: {statusResp.OperationState}");
                    }
                    else
                    {
                        OutputHelper?.WriteLine("OperationHandle is null after close");
                    }
                }
                catch (Exception ex)
                {
                    OutputHelper?.WriteLine($"Cannot check status after close: {ex.Message}");
                }
            }
            finally
            {
            ...
            OutputHelper?.WriteLine("=== Test 3 Complete ===");
        }
    }
}