...
namespace Apache.Arrow.Adbc.Tests.Drivers.Apache.Spark.CloudFetch
{
    ...
    public class CloudFetchResultFetcherTest
    {
        ...
        /// <summary>
        /// Test implementation of IHiveServer2StatementProperties for testing
        /// </summary>
        public class TestHiveServer2Statement : ITestableHiveServer2Statement
        {
            private readonly TOperationHandle _operationHandle;
            private Func<TFetchResultsReq, CancellationToken, Task<TFetchResultsResp>>? _fetchResultsFunc;
            private Queue<(List<TSparkArrowResultLink> resultLinks, bool hasMoreRows)>? _fetchResultsSequence;
            private Exception? _fetchResultsException;
            
            public int FetchResultsCallCount { get; private set; }
            
            public TestHiveServer2Statement()
            {
            ...
            public TOperationHandle? OperationHandle => _operationHandle;
            
            public long BatchSize => 1000;
            
            public TCLIService.Client Client => throw new NotImplementedException("This should not be called directly");
            
            public void SetupFetchResults(List<TSparkArrowResultLink> resultLinks, bool hasMoreRows)
            {
                _fetchResultsFunc = (req, token) =>
                {
                    FetchResultsCallCount++;
                    var response = CreateFetchResultsResponse(resultLinks, hasMoreRows);
                    return Task.FromResult(response);
                };
                _fetchResultsSequence = null;
                _fetchResultsException = null;
            }
            
            public void SetupFetchResultsSequence(IEnumerable<(List<TSparkArrowResultLink> resultLinks, bool hasMoreRows)> sequence)
            {
                _fetchResultsSequence = new Queue<(List<TSparkArrowResultLink> resultLinks, bool hasMoreRows)>(sequence);
                _fetchResultsFunc = null;
                _fetchResultsException = null;
            }
            
            public void SetupFetchResultsThrows(Exception exception)
            {
                _fetchResultsException = exception;
                _fetchResultsFunc = null;
                _fetchResultsSequence = null;
            }
            
            public void SetupFetchResultsCallback(Func<TFetchResultsReq, CancellationToken, Task<TFetchResultsResp>> callback)
            {
                _fetchResultsFunc = callback;
                _fetchResultsSequence = null;
                _fetchResultsException = null;
            }
            
            public Task<TFetchResultsResp> FetchResults(TFetchResultsReq req, CancellationToken cancellationToken)
            {
                FetchResultsCallCount++;
                
                if (_fetchResultsException != null)
                {
                    return Task.FromException<TFetchResultsResp>(_fetchResultsException);
                }
                
                if (_fetchResultsFunc != null)
                {
                    return _fetchResultsFunc(req, cancellationToken);
                }
                
                if (_fetchResultsSequence != null && _fetchResultsSequence.Count > 0)
                {
                    var (resultLinks, hasMoreRows) = _fetchResultsSequence.Dequeue();
                    return Task.FromResult(CreateFetchResultsResponse(resultLinks, hasMoreRows));
                }
                
                // Default empty response
                return Task.FromResult(CreateFetchResultsResponse(new List<TSparkArrowResultLink>(), false));
            }
