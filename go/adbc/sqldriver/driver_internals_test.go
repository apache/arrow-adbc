...
func TestArrFromVal(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedStringValue: testTime.UTC().Truncate(time.Second).Format("2006-01-02T15:04:05"),
		},
		{
			value:               tstampMilli,
			inputDataType:       &arrow.TimestampType{Unit: arrow.Millisecond},
			expectedDataType:    &arrow.TimestampType{Unit: arrow.Millisecond},
			expectedStringValue: testTime.UTC().Truncate(time.Millisecond).Format("2006-01-02T15:04:05.000"),
		},
		{
			value:               tstampMicro,
			inputDataType:       &arrow.TimestampType{Unit: arrow.Microsecond},
			expectedDataType:    &arrow.TimestampType{Unit: arrow.Microsecond},
			expectedStringValue: testTime.UTC().Truncate(time.Microsecond).Format("2006-01-02T15:04:05.000000"),
		},
		{
			value:               tstampNano,
			inputDataType:       &arrow.TimestampType{Unit: arrow.Nanosecond},
			expectedDataType:    &arrow.TimestampType{Unit: arrow.Nanosecond},
			expectedStringValue: testTime.UTC().Truncate(time.Nanosecond).Format("2006-01-02T15:04:05.000000000"),
		},
		{
			value:               testTime,
			inputDataType:       &arrow.TimestampType{Unit: arrow.Second},
