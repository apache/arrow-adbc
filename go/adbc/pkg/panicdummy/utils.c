...
void PanicDummyReleaseErrWithDetails(struct AdbcError* error) {
  if (!error || error->release != PanicDummyReleaseErrWithDetails ||
      !error->private_data) {
    return;
  }

  struct PanicDummyError* details =
      (struct PanicDummyError*) error->private_data;
...
int PanicDummyErrorGetDetailCount(const struct AdbcError* error) {
  ...
  return ((struct PanicDummyError*) error->private_data)->count;
}

struct AdbcErrorDetail PanicDummyErrorGetDetail(const struct AdbcError* error,
                                                 int index) {
  if (!error || error->release != PanicDummyReleaseErrWithDetails ||
      !error->private_data) {
    return (struct AdbcErrorDetail){NULL, NULL, 0};
  }
  struct PanicDummyError* details = (struct PanicDummyError*) error->private_data;
...
struct AdbcErrorDetail AdbcErrorGetDetail(const struct AdbcError* error, int index) {
  return PanicDummyErrorGetDetail(error, index);
}

const struct AdbcError* AdbcErrorFromArrayStream(struct ArrowArrayStream* stream,
                                                 AdbcStatusCode* status) {
  return PanicDummyErrorFromArrayStream(stream, status);
}
...
AdbcStatusCode AdbcConnectionGetOptionBytes(struct AdbcConnection* connection,
  ...
  return PanicDummyConnectionGetOptionBytes(connection, key, value, length, error);
}
...
AdbcStatusCode AdbcConnectionGetStatistics(struct AdbcConnection* connection,
                                           ...
                                           struct ArrowArrayStream* out,
                                           struct AdbcError* error) {
  return PanicDummyConnectionGetStatistics(connection, catalog, db_schema, table_name,
...
AdbcStatusCode AdbcConnectionSetOptionBytes(struct AdbcConnection* connection,
                                            ...
                                            size_t length, struct AdbcError* error) {
  return PanicDummyConnectionSetOptionBytes(connection, key, value, length, error);
}
...
AdbcStatusCode AdbcStatementBind(struct AdbcStatement* statement,
                                 ...
                                 struct AdbcError* error) {
  return PanicDummyStatementBind(statement, values, schema, error);
}
...
AdbcStatusCode AdbcStatementExecutePartitions(struct AdbcStatement* statement,
                                              ...
                                              struct AdbcError* error) {
  return PanicDummyStatementExecutePartitionsTrampoline(
...
AdbcStatusCode AdbcStatementGetOptionBytes(struct AdbcStatement* statement,
                                           const char* key, uint8_t* value,
                                           size_t* length, struct AdbcError* error) {
  return PanicDummyStatementGetOptionBytes(statement, key, value, length, error);
}
...
AdbcStatusCode AdbcStatementSetOptionBytes(struct AdbcStatement* statement,
                                           const char* key, const uint8_t* value,
                                           size_t length, struct AdbcError* error) {
  return PanicDummyStatementSetOptionBytes(statement, key, value, length, error);
}
...
int PanicDummyArrayStreamGetSchemaTrampoline(struct ArrowArrayStream* stream,
  ...
  return PanicDummyArrayStreamGetSchema(stream, out);
}
...
int PanicDummyArrayStreamGetNextTrampoline(struct ArrowArrayStream* stream,
  ...
  // XXX(https://github.com/apache/arrow-adbc/issues/729)
  memset(out, 0, sizeof(*out));
  return PanicDummyArrayStreamGetNext(stream, out);
}
