...
type DriverQuirks interface {
	...
	// Create a sample table from an arrow record with metadata
	CreateSampleTableWithMetadata(tableName string, r arrow.Record) error
...
func (c *ConnectionTests) TestMetadataGetObjectsColumnsXdbc() {
	ctx := context.Background()
	cnxn, _ := c.DB.Open(ctx)
	defer cnxn.Close()
	...
	c.Require().NoError(c.Quirks.DropTable(cnxn, ingestTableName))
	bigIntCol := make(map[string]string)
	bigIntCol[COLUMN_NAME] = "bigIntCol"
	bigIntCol[ORDINAL_POSITION] = "1"
	bigIntCol[REMARKS] = "this is a big int col"
	bigIntCol[XDBC_DATA_TYPE] = strconv.Itoa(int(arrow.PrimitiveTypes.Int64.ID()))
	bigIntCol[XDBC_TYPE_NAME] = "BIGINT"
	bigIntCol[XDBC_COLUMN_SIZE] = "19"
	bigIntCol[XDBC_DECIMAL_DIGITS] = ""
	bigIntCol[XDBC_NUM_PREC_RADIX] = ""
	bigIntCol[XDBC_NULLABLE] = ""
	bigIntCol[XDBC_COLUMN_DEF] = ""
	bigIntCol[XDBC_SQL_DATA_TYPE] = strconv.Itoa(int(arrow.PrimitiveTypes.Int64.ID()))
	bigIntCol[XDBC_DATETIME_SUB] = ""
	bigIntCol[XDBC_CHAR_OCTET_LENGTH] = ""
	bigIntCol[XDBC_IS_NULLABLE] = ""
	bigIntCol[XDBC_SCOPE_CATALOG] = ""
	bigIntCol[XDBC_SCOPE_SCHEMA] = ""
	bigIntCol[XDBC_SCOPE_TABLE] = ""
	bigIntCol[XDBC_IS_AUTOINCREMENT] = ""
	bigIntCol[XDBC_IS_AUTOGENERATEDCOLUMN] = ""

	rec, _, err := array.RecordFromJSON(c.Quirks.Alloc(), arrow.NewSchema(
		[]arrow.Field{{
			Name:     bigIntCol[COLUMN_NAME],
			Type:     arrow.PrimitiveTypes.Int64,
			Nullable: false,
			Metadata: arrow.MetadataFrom(bigIntCol),
		},
		}, nil), strings.NewReader(`[
			{"bigIntCol": -9223372036854775808},
	...
	// mdInts["XDBC_SQL_DATA_TYPE"] = strconv.Itoa(int(XdbcDataType_XDBC_BIGINT))
	...
	// mdStrings["XDBC_SQL_DATA_TYPE"] = strconv.Itoa(int(XdbcDataType_XDBC_VARCHAR))
	...
	c.Require().NoError(c.Quirks.CreateSampleTableWithMetadata("bulk_ingest", rec))
	...
	for _, tt := range tests {
		c.Run(tt.name, func() {
			rdr, err := cnxn.GetObjects(ctx, adbc.ObjectDepthColumns, nil, nil, nil, nil, nil)
			c.Require().NoError(err)
			defer rdr.Release()
			...
			c.Equal(tt.columnName, columnName)
			c.Equal(tt.ordinalPosition, ordinalPosition)
			c.Equal(tt.remarks, remarks)
			c.Equal(tt.xdbcDataType, xdbcDataType)
			c.Equal(tt.xdbcTypeName, xdbcTypeName)
			c.Equal(tt.xdbcColumnSize, xdbcColumnSize)
			c.Equal(tt.xdbcDecimalDigits, xdbcDecimalDigits)
			c.Equal(tt.xdbcNumPrecRadix, xdbcNumPrecRadix)
			c.Equal(tt.xdbcNullable, xdbcNullable)
			c.Equal(tt.xdbcColumnDef, xdbcColumnDef)
			c.Equal(tt.xdbcSqlDataType, xdbcSqlDataType)
			c.Equal(tt.xdbcDatetimeSub, xdbcDatetimeSub)
			c.Equal(tt.xdbcCharOctetLength, xdbcCharOctetLength)
			c.Equal(tt.xdbcIsNullable, xdbcIsNullable)
			c.Equal(tt.xdbcScopeCatalog, xdbcScopeCatalog)
			c.Equal(tt.xdbcScopeSchema, xdbcScopeSchema)
			c.Equal(tt.xdbcScopeTable, xdbcScopeTable)
			c.Equal(tt.xdbcIsAutoincrement, xdbcIsAutoincrement)
			c.Equal(tt.xdbcIsAutogeneratedColumn, xdbcIsAutogeneratedColumn)
