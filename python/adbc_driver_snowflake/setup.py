...
library = os.environ.get("ADBC_SNOWFLAKE_LIBRARY")
...
if not library:
    if os.environ.get("_ADBC_IS_SDIST", "").strip().lower() in ("1", "true"):
        print("Building sdist, not requiring ADBC_SNOWFLAKE_LIBRARY")
    elif os.environ.get("_ADBC_IS_CONDA", "").strip().lower() in ("1", "true"):
        print("Building Conda package, not requiring ADBC_SNOWFLAKE_LIBRARY")
    elif target.is_file():
        print("Driver already exists (but may be stale?), continuing")
    else:
        raise ValueError("Must provide ADBC_SNOWFLAKE_LIBRARY")
else:
...
version, cmdclass = get_version_and_cmdclass("adbc_driver_snowflake")
