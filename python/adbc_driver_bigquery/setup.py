...
library = os.environ.get("ADBC_BIGQUERY_LIBRARY")
target = source_root.joinpath(
    "./adbc_driver_bigquery/libadbc_driver_bigquery.so"
).resolve()
if not library:
    if os.environ.get("_ADBC_IS_SDIST", "").strip().lower() in ("1", "true"):
        print("Building sdist, not requiring ADBC_BIGQUERY_LIBRARY")
    elif os.environ.get("_ADBC_IS_CONDA", "").strip().lower() in ("1", "true"):
        print("Building Conda package, not requiring ADBC_BIGQUERY_LIBRARY")
    elif target.is_file():
        print("Driver already exists (but may be stale?), continuing")
    else:
        raise ValueError("Must provide ADBC_BIGQUERY_LIBRARY")
else:
...
version = get_version("adbc_driver_bigquery")
