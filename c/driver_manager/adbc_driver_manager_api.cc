...
static const char* ErrorArrayStreamGetLastError(struct ArrowArrayStream* stream) {
...
static int ErrorArrayStreamGetNext(struct ArrowArrayStream* stream,
...
static int ErrorArrayStreamGetSchema(struct ArrowArrayStream* stream,
...
static void ErrorArrayStreamInit(struct ArrowArrayStream* out,
                                 struct AdbcDriver* private_driver) {
  if (!out || !out->release ||
...
AdbcStatusCode AdbcDriverManagerDatabaseSetLoadFlags(struct AdbcDatabase* database,
  ...
  TempDatabase* args = reinterpret_cast<TempDatabase*>(database->private_data);
  args->load_flags = load_flags;
  return ADBC_STATUS_OK;
}

AdbcStatusCode AdbcDriverManagerDatabaseSetAdditionalSearchPathList(
    struct AdbcDatabase* database, const char* additional_search_path_list,
    struct AdbcError* error) {
  if (database->private_driver) {
...
// AdbcProfileProviderFilesystem is implemented in the profiles file
...
AdbcStatusCode AdbcConnectionGetStatistics(struct AdbcConnection* connection,
  ...
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetStatistics(
...
AdbcStatusCode AdbcConnectionGetTableTypes(struct AdbcConnection* connection,
  ...
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetTableTypes(connection, out, error),
              out, connection);
}
