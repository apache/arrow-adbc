...
constexpr std::array<PostgresTypeResolver::Item, {item_count}> kBuiltinTypeItems = {{{{
{items}
}}}};
...
def generate(defs) -> str:
    ...
    for defn in defs:
        ...
        lines.append(
            f'    {{{defn.oid}, "{defn.typname}", "{defn.typreceive}", 0, 0, 0}},'
        )

        if defn.array_type_oid:
            lines.append(
                f'    {{{defn.array_type_oid}, "_{defn.typname}", "array_recv", '
                f"{defn.oid}, 0, 0}},"
            )

    return template.format(item_count=len(lines), items="\n".join(lines)).strip()
