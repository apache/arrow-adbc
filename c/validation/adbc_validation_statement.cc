...
void StatementTest::TestSqlIngestStreamZeroArrays() {
  ...
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement,
          ("SELECT * FROM " + quirks()->QuoteIdentifier("bulk_ingest")).c_str(), &error),
      IsOkStatus(&error));
...
void StatementTest::TestSqlPrepareUpdate() {
  ...
  // Prepare
  std::string query = "INSERT INTO " + quirks()->QuoteIdentifier("bulk_ingest") +
...
void StatementTest::TestSqlQueryInsertRollback() {
  ...
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement, ("CREATE TABLE " + table + " (a INT)").c_str(), &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
...
void StatementTest::TestSqlQueryRowsAffectedDelete() {
  ...
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement, ("CREATE TABLE " + table + " (foo INT)").c_str(), &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
